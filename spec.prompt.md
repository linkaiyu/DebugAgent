...
2. **Granular Specification Nodes (`specs/*.spec.json`)**: Individual JSON files that hold structural descriptions and pointer fields tracking down to exact files and function names. Spec files should follow the naming convention `specs/<python code file name>.spec.json`. **Read these only when targeted by the graph engine.**
3. **Application Source Code**: Clean execution code containing a lightweight `@spec <id>` docstring marker pointing back to its governing node.
...
*   `python spec_tool.py --index-file <name>.json search --id <node_id>` -> Fetches node metadata and relationship edges for a node.
*   `python spec_tool.py --index-file <name>.json node --id <node_id>` -> Inspect a graph node and show incoming/outgoing semantic relationships.
...
*   `python spec_tool.py --index-file <name>.json update --id <spec_id> --summary "<text>" --depends_on "<dep1,dep2>" --file "<path>" --function "<name>"` -> Creates or edits a specification node and regenerates the graph.
...
1. Call `python spec_tool.py --index-file <relevant_index_file>.json update` to rewrite the modified logic summary and update dependency matrices inside the `.spec.json` layer.
